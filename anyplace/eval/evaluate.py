...
def main(args: config_util.AttrDict) -> None:
    ...
    tasks = ['hang_mug', 'stack_blocks', 'pour_into_bowl', 'push_T_hex_key', 'close_lid']
    for task in tasks:
        eval_dataset_path = args.experiment.eval.eval_dataset_path + task + '_test_data'
        ...
        for data_path in eval_dataset_list:
            ...
            for iteration in range(exp_args.start_iteration, exp_args.num_iterations):
                ...
                save_data_dir = "/home/jiahe/anyplace_result/" + task + '/' 
                OUTPUT_DIR = Path(save_data_dir)
                OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

                np.save(  save_data_dir+ f"test{str(data_path)[-6:-4]}.npy", result, allow_pickle = True)
