...
class TransformChamferWrapper:
    ...
    def tf_chamfer(self, model_outputs, ground_truth, quat_norm_beta=0.1, val=False):
        ...
        loss_dict['trans_L1'] = torch.abs(trans_label - model_outputs['trans']).mean()
        # rotation loss (geodesic, and encourage normalized quat)
        # rot_loss = torch.norm(quat_label - model_outputs[''], p=2, dim=-1).mean()
        # rot_loss = torch.mean(torch.pow(model_outputs['quat'] - quat_label, 2))  # MSE 
        ...
        # Compute MSE between rotation matrices
        # predicted = model_outputs['rot_mat']
        # print(f'predicted:{predicted}')
        # print(f'rot_label: {rot_mat_label}')
        # rot_loss = torch.nn.functional.mse_loss(model_outputs['rot_mat'], rot_mat_label)
        ...
        loss_dict['rot'] = rot_loss
        loss_dict['chamf'] = chamf_loss
        #loss_dict['rot_quat_dist'] = quat_dist.mean()
        #loss_dict['rot_quat_norm_loss'] = norm_loss.mean()
        #loss_dict['trans_mean'] = torch.norm(model_outputs['trans'], dim=1).mean()
